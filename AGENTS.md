...
```
src/
├── App.jsx                      # Root — owns all state (search, category, booksList, drawer, roulette)
...
│
├── data/
│   ├── bookTags.js              # Tags per book
│   ├── csvString.js             # Raw CSV exported as `csvData` (486 KB — do NOT edit manually)
│   ├── parsedBooks.js           # CSV parser → exports `books` (array), `categories` (Set), `recommenders` (Set)
│   └── recommendations.js       # Curated recommendation notes
│
└── hooks/
...
const [selectedRecommender, setSelectedRecommender]; // active recommender filter
const [booksList, setBooksList]; // ordered book array (mutated by Shuffle)
const [selectedBook, setSelectedBook]; // book selected for detail drawer
```

**`filteredBooks`** (memoized): Applies category filter, recommender filter, then search filter across `title`, `author`, `category`, `summary`, `recommender`, and `tags`. Passed to `<BookGrid>` and `<BookRouletteModal>`.

**Shuffle**: Fisher-Yates on `booksList`, then resets filters.

---

## Animation Architecture (`BookGrid.jsx` & `BookRouletteModal.jsx`)
...
- `BookRouletteModal`:
  - Rotating SVG segmented wheel with pointer physics (needle wiggling on sector bounds).
  - Web Audio API ticker click sounds and celebratory victory chime upon stopping.
  - Framer Motion confetti burst + spring entrance card displaying the winner.
...
- Recommender: `<Select>` with curated recommenders
- Shuffle: amber CTA button calls `onShuffle` in App
- Roulette: opens `BookRouletteModal` with candidate books

### `BookRouletteModal.jsx`

- Interactive spinning wheel with dynamic book candidate slices
- Synthesizes audio clicks and victory chime via Web Audio API (zero dependencies)
- Offers direct actions to view full details or visit external link
